...
public class FoodMasterBot extends TelegramLongPollingBot {
    ...
    private void requestPhoto(long chatId) {
        SendMessage message = new SendMessage();
        message.setChatId(chatId);
        message.setText("📸 Пожалуйста, отправьте фото блюда.");
    ...
    private void processPhoto(Message message) {
        ...
        String fileId = message.getPhoto().get(message.getPhoto().size() - 1).getFileId(); // Берем лучшее качество

        sendMessage(chatId, "Обрабатываю фото...");

        String imageUrl = getFileUrl(fileId);
        if (imageUrl == null) {
            sendMessage(chatId, "Ошибка при получении фото, попробуйте снова.");
            return;
        }

        // Анализ фото через OpenAI API
    ...
    private String getFileUrl(String fileId) {
        String fileUrl = "https://api.telegram.org/bot" + getBotToken() + "/getFile?file_id=" + fileId;

        RestTemplate restTemplate = new RestTemplate();
        ResponseEntity<Map> response = restTemplate.getForEntity(fileUrl, Map.class);
    ...
    private String formatNutritionData(Map<String, Object> nutritionData) {
        StringBuilder result = new StringBuilder("📊 *Примерное КБЖУ блюда:*\n\n");
        nutritionData.forEach((key, value) -> result.append("▫ ").append(key).append(": *").append(value).append("*\n"));
    ...
    public void onUpdateReceived(Update update) {
        if (update.hasMessage()) {
            ...
            if ("CALCULATE_KBZU_RECIPE".equals(callbackData)) {
                requestPhoto(chatId);
            }
        }
